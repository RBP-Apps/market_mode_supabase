    ...
    performanceData.forEach(item => {
      if (!item.error) {
        syncData.push({
          ...
          inverter_id: item.inverterId.trim(),
          beneficiary_name: item.beneficiaryName.trim(),
          capacity_kw: item.capacity,
          total_energy_kwh: item.totalKwh,
          avg_daily_energy_kwh: item.avgDailyKwh,
          specific_yield: item.specYield,
          days_in_range: item.daysInRange || calculateDaysInRange(),
          lifetime_generation_kwh: item.lifetimeGeneration || 0,
