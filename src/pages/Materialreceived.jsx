...
function MaterialReceivedSitePage() {
  ...
  const [fileUploads, setFileUploads] = useState({
    ...
    moduleDamage: { uploading: false, uploaded: false, url: "", error: null, name: "" }
  })
  ...
  const fetchSheetData = useCallback(async () => {
    try {
      ...
      // 🔹 1. fetch material_receipts and quotation in parallel
      const [{ data: mrData, error: mrError }, { data: quotationData, error: quotationError }] =
        await Promise.all([
          supabase
      ...
      (mrData || []).forEach((row) => {
        ...
        const rowData = {
          ...
          moduleDamage: row.module_damage || "",
          remark: row.remark || "",
        }
  ...
  const handleAtSiteClick = useCallback((record) => {
    ...
    setFileUploads({
      ...
      moduleDamage: {
        uploading: false,
        uploaded: !!record.moduleDamage,
        url: record.moduleDamage || "",
        error: null,
        name: record.moduleDamage ? "Existing Document" : ""
      }
    })
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          module_damage: fileUploads.moduleDamage.url || null,
                    ...
                    filteredHistoryData.length > 0 ? (
                      filteredHistoryData.map((record) => (
                        <tr key={record._id} className="hover:bg-gray-50">
                          ...
                          <td className="px-2 py-3 max-w-xs">
                            <div className="text-xs text-gray-900 whitespace-normal break-words" title={record.remark}>
                              {record.remark || "—"}
                            </div>
                          </td>
                        </tr>
            ...
            <div className="relative bg-white border max-w-4xl w-full shadow-2xl rounded-lg max-h-[90vh] overflow-y-auto">
              ...
              <div className="p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  {/* Module Damage Upload */}
                  <div className="md:col-span-2">
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      Module Damage File <span className="text-gray-500 text-xs ml-1">(Optional, Image/PDF)</span>
                    </label>
                    <input
                      type="file"
                      accept="image/*,.pdf"
                      onChange={(e) => handleFileUpload("moduleDamage", e.target.files[0])}
                      className="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100"
                    />
                    <UploadStatus field="moduleDamage" />

                    {selectedRecord?.moduleDamage && (
                      <div className="mt-2 flex items-center space-x-2">
                        <span className="text-xs text-gray-500">Existing:</span>
                        <button
                          type="button"
                          onClick={() => window.open(selectedRecord.moduleDamage, "_blank", "noopener,noreferrer")}
                          className="inline-flex items-center px-2 py-1 text-xs font-medium rounded-md text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition-colors"
                        >
                          <Eye className="h-3 w-3 mr-1" />
                          Preview Existing Damage Document
                        </button>
                      </div>
                    )}
                  </div>
                ...
                <div className="flex justify-end space-x-4 mt-8 pt-4 border-t">
                  ...
                  <button
                    ...
                    disabled={isSubmitting || (!fileUploads.copyOfReceipt.url && !selectedRecord?.copyOfReceipt) || !receiptForm.dateOfReceipt}
