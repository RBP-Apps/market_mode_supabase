...
export default async function handler(req, res) {
    ...
    try {
        ...
        // 6. Push collected data to Supabase
        ...
        const recordsToInsert = performanceData.map(data => ({
            serial_no: parseInt(String(data.serialNo).replace(/\D/g, '')) || null,
            inverter_id: data.inverterId,
            beneficiary_name: data.beneficiaryName,
            capacity_kw: data.capacity,
            total_energy_kwh: data.totalKwh,
            avg_daily_energy_kwh: data.avgDailyKwh,
            specific_yield: data.specYield,
            days_in_range: data.daysInRange,
            lifetime_generation_kwh: data.lifetimeGeneration,
